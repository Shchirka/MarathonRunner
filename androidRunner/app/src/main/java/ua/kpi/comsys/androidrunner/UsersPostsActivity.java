...
public class UsersPostsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        progressDialog = new ProgressDialog(UsersPostsActivity.this);
        progressDialog.setTitle("Downloading your posts...");

        user = FirebaseAuth.getInstance().getCurrentUser();
        userID = user.getUid();
        referencePost = FirebaseDatabase.getInstance().getReference("Posts").child(userID);
        ...
        recyclerView = (RecyclerView) findViewById(R.id.user_posts_list);
    }
    ...
    private void setInitialData() throws IOException {
        if(collectPosts.size() == 0){
            ...
            Toast.makeText(this, "You don't have any posts yet", Toast.LENGTH_SHORT).show();
        }
        else{
    ...
    private void setData(int index){
        ...
        referenceSinglePost.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                //new AsyncRequest().execute(userPost.imageURL);
                try {
                    userBitmap = HomeActivity.accountPhoto.getDrawingCache();
                    ...
                    if(posts.size() == collectPosts.size()){
                        progressDialog.dismiss();
                        postAdapter = new PostAdapter(UsersPostsActivity.this, posts);
